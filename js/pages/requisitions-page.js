...
async function openRequisitionForm(container) {
  ...
  form.innerHTML = `
    ...
    <div class="dialog__actions"><button class="button button--secondary" type="button" data-requisition-cancel>Cancel</button><button class="button" type="submit">Raise requisition</button></div>`;
  let modal;
...
export async function renderRequisitionsPage(container) {
  ...
  const rows = requisitions.length ? requisitions.map((requisition) => `<tr><td><a class="component-name-link" href="/requisitions/${requisition.id}" data-route-link>${requisition.id}</a></td><td>${escapeHtml(requisitionName(requisition))}</td><td>${formatQuantity(requisition.quantity, requisition.unit?.symbol)}</td><td>${escapeHtml(requisition.requester?.name || '—')}</td><td>${escapeHtml(requisition.project?.name || '—')}</td><td><span class="status-badge status-badge--${statusClass(requisition.status)}">${requisition.status}</span></td><td>${formatDate(requisition.createdOn)}</td><td>${formatDate(requisition.neededBy)}</td><td>${requisition.daysInStatus} day${requisition.daysInStatus === 1 ? '' : 's'}</td></tr>`).join('') : '<tr><td colspan="9">No requisitions match this filter.</td></tr>';
  container.innerHTML = `<section class="requisitions-page" aria-label="Requisitions"><div class="route-actions"><button class="button" type="button" data-create-requisition>Raise requisition</button></div><section class="requisition-toolbar"><label class="field"><span class="visually-hidden">Filter requisitions by status</span><select class="field__control" data-requisition-filter><option value="all">All statuses</option>${['Pending', 'Approved', 'Ordered', 'Received', 'Rejected', 'Cancelled'].map((status) => `<option value="${status}" ${requisitionFilter === status ? 'selected' : ''}>${status}</option>`).join('')}</select></label></section><div class="library-table-wrap"><table class="library-table"><thead><tr><th>Request ID</th><th>Component / part</th><th>Quantity</th><th>Requested by</th><th>Project</th><th>Status</th><th>Created</th><th>Needed by</th><th>Age</th></tr></thead><tbody>${rows}</tbody></table></div></section>`;
...
export async function renderRequisitionDetailsPage(container, requisitionId) {
  ...
  if (!requisition) { container.innerHTML = '<section class="state-panel"><h2 class="state-panel__title">Requisition not found</h2><a class="button" href="/requisitions" data-route-link>Back to Requisitions</a></section>'; return; }
  ...
  container.innerHTML = `<section class="requisition-details-page" aria-labelledby="requisition-title"><a class="back-link" href="/requisitions" data-route-link>Back to Requisitions</a><header class="overview-header"><div><p class="eyebrow">${requisition.id}</p><h2 id="requisition-title">${escapeHtml(requisitionName(requisition))}</h2><p>${formatQuantity(requisition.quantity, requisition.unit?.symbol)} · ${escapeHtml(requisition.project?.name || 'No project')}</p></div><div class="requisition-detail-actions"><span class="status-badge status-badge--${statusClass(requisition.status)}">${requisition.status}</span>${canUpdate ? '<button class="button" type="button" data-update-requisition-status>Update status</button>' : ''}</div></header><div class="requisition-details-grid"><section class="detail-card"><h3>Request details</h3><dl class="detail-list"><div><dt>Requested by</dt><dd>${escapeHtml(requisition.requester?.name || '—')}</dd></div><div><dt>Created</dt><dd>${formatDateTime(requisition.createdOn)}</dd></div><div><dt>Needed by</dt><dd>${formatDate(requisition.neededBy)}</dd></div><div><dt>Note</dt><dd>${escapeHtml(requisition.note || 'No note')}</dd></div></dl></section><section class="detail-card"><h3>Status history</h3><ol class="status-history">${history}</ol></section></div></section>`;
  requisitionController = new AbortController();
